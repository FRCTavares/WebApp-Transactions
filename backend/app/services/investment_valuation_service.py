"""Valuation and analytics logic for investment events.

Split out of `investment_event_service.py` (which was approaching the
project's 1,000-line hard limit) to keep CRUD/event-mutation logic separate
from portfolio valuation, cost-basis, and FX-rate calculations. This is a
mixin consumed by `InvestmentEventService`, not a standalone service — it
relies on `self.repository`, `self.market_price_repository`, and
`self.market_price_history_repository` being set by that class's
`__init__`.
"""
